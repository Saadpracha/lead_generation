...
class YpCategoriesSpider(scrapy.Spider):
    ...
    def parse_level_3(self, response):
        ...
        for name, rel_url in zip(names, urls):
            # Skip URLs that contain '/products/'
            if '/products/' in rel_url:
                continue
            ...
            if self.lang == 'en':
                data.update({
                    ...
                    "level_3_name_en": name.strip(),
                    "leaf_name_en": name.strip(),
                    "source_path_en": " > ".join(response.meta["breadcrumb"]),
                })
            elif self.lang == 'fr':
                data.update({
                    "level_1_name_fr": response.meta.get("level_1_name_fr"),
                    "level_2_name_fr": response.meta.get("level_2_name_fr"),
                    "level_3_name_fr": name.strip(),
                    "leaf_name_fr": name.strip(),
                    "source_path_fr": " > ".join(response.meta["breadcrumb"]),
                })
